...
export function registerContinueFromTool(server: any) {
  server.tool(
    "continue_from",
    "Get handoff context from a previous agent's work — recent transcript, events, and files — so you can continue from where the named agent left off. Works for live and stopped agents. Returns the bundle as JSON; you decide how to phrase the continuation. The tool does not launch or mutate anything. Read-only; no sender identity required. Related: resume (continue the session), fork (branch it).",
    {
