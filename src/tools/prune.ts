...
export function registerPruneTool(server: any) {
  server.tool(
    "prune",
    "Remove stale registry records. Reconciles against live hcom state first (never-live records are demoted to lost where the age rules can reach them), then applies the age rules. By default targets managed_lost and adopted_lost records older than 7 days (dry-run only). Use confirm=true to actually remove records. Use includeStopped=true to also target managed_stopped and adopted_stopped records older than 30 days. Use allWorkspaces=true to prune every workspace in one call instead of one call per workspace.",
    {
